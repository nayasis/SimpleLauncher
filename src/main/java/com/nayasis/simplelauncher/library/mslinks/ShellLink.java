 ...
 *
 * <pre>
 * it can make or read link
 *
 * https://github.com/BlackOverlord666/mslinks
 * </pre>
 * @author https://github.com/BlackOverlord666
 *
 */
public class ShellLink {
	...
	public ShellLink(InputStream in) throws ShellLinkException, IOException {
		init( in );
	}

	private void init( InputStream in ) throws ShellLinkException, IOException {

		// 오류 발생시 InputStrem 자원을 반환하지 않는 문제 수정

		try {
			init( new ByteReader(in) );
        } catch( ShellLinkException | IOException e ) {
	        throw e;
        } finally {
        	in.close();
        }
    ...
    private void init( ByteReader data ) throws ShellLinkException, IOException {
	...
	/**
	 * Get target path of <b>*.lnk</b> file
	 *
	 * @return Target path of LNK
	 */
	public String getTargetPath() {
		...
		if( info.getCommonNetworkRelativeLink() == null ) {

			sb.append( info.getLocalBasePath() );

		} else {

			sb.append( info.getCommonNetworkRelativeLink().getNetName() );
			sb.append( '\\' );
			sb.append( info.getCommonPathSuffix() );
